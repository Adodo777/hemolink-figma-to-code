        ...
        <div className="w-full lg:w-1/3 lg:sticky lg:top-32 lg:self-start">
          ...
          <div className="mt-8 lg:mt-10 relative">
            <select
              ...
              className="w-full appearance-none bg-transparent border-b-2 border-blood-gray-200 text-zinc-950 font-bold text-lg py-4 pr-12 rounded-none focus:outline-none focus:border-blood-600 transition-all cursor-pointer"
            >
          ...
          <div className="mt-8 space-y-3">
            <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-wider">Filtres</h3>
            <div className="flex flex-wrap gap-4">
              <button
                onClick={() => setFilterOpen(!filterOpen)}
                className={`pb-2 rounded-none text-sm font-bold transition-all border-b-2 ${filterOpen ? 'text-emerald-600 border-emerald-600' : 'text-zinc-500 border-blood-gray-300'}`}
              >
                Ouvert
              </button>
              <button
                onClick={() => setFilterPlasma(!filterPlasma)}
                className={`pb-2 rounded-none text-sm font-bold transition-all border-b-2 ${filterPlasma ? 'text-blood-600 border-blood-600' : 'text-zinc-500 border-blood-gray-300'}`}
              >
                Don de plasma
              </button>
              <button
                onClick={() => setFilterNoAppt(!filterNoAppt)}
                className={`pb-2 rounded-none text-sm font-bold transition-all border-b-2 ${filterNoAppt ? 'text-blood-600 border-blood-600' : 'text-zinc-500 border-blood-gray-300'}`}
              >
                Sans rendez-vous
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            {activeCenters.length === 0 ? (
              <div className="bg-blood-gray-50 rounded-[32px] p-10 text-center border border-blood-gray-100">
                <p className="text-zinc-500 font-medium">Aucun centre ne correspond à vos critères dans ce département.</p>
                <button
              ...
              activeCenters.map((center) => (
                <div key={center.id} className="relative py-8 sm:py-10 border-b border-blood-gray-200 last:border-0 group flex flex-col transition-all duration-500">
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                    <span className="bg-blood-gray-50 text-zinc-500 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border border-blood-gray-100">
                      {center.city}
                    </span>
                    {center.isOpen ? (
                      ...
                      <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider bg-blood-gray-50 px-4 py-1.5 rounded-full border border-blood-gray-100">Fermé</span>
                    )}
                  </div>

                  <h3 className="text-xl sm:text-2xl font-bold text-zinc-950 leading-snug mb-4 group-hover:text-blood-600 transition-colors pr-8">
                    {center.name}
                  </h3>
                  ...
                  <div className="flex flex-wrap gap-2 mb-8 pr-16 sm:pr-24">
                    {center.donationTypes.map((type) => (
                      <span key={type} className="px-3 py-1 text-zinc-600 text-xs font-bold border-b border-blood-gray-200">
                        {type}
                      </span>
                    ))}
                    <span className="px-3 py-1 text-blood-600 text-xs font-bold">
                      {center.appointment}
                    </span>
                  </div>

                  <div className="space-y-5 text-zinc-600 flex-1 pb-14 sm:pb-0 pr-0 sm:pr-32">
                    <div className="flex items-start gap-4">
                      <div className="bg-blood-gray-50 p-3 rounded-2xl text-zinc-400 shrink-0 group-hover:bg-blood-50 group-hover:text-blood-600 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <p className="text-sm font-bold text-zinc-700">{center.phone}</p>
                    </div>
                  </div>
                  ...
                  <div className="absolute bottom-8 sm:bottom-10 right-0 flex gap-3">
                    {center.mapUrl && (
                      <a
                        ...
                        title="Itinéraire"
                        className="w-12 h-12 rounded-full bg-white text-zinc-400 hover:text-blood-600 hover:bg-blood-50 border border-blood-gray-200 hover:border-blood-100 transition-all flex items-center justify-center group/btn shadow-sm"
                      >
